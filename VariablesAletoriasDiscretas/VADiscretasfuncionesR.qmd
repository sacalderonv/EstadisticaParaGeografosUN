...
**Tarea:** Cómo se calcularía la probabilidad de hayan entre 3 y 7 árboles de totumo, inclusive?
...
Recordemos el ejemplo relacionado con $X$ el número de nacimientos al día en Colombia, donde 41.8 es el número de nacimiento promedio en un día cualquiera. En este caso parece razonable pensar que $X\sim Poisson(\lambda=41.8).$ Cómo calculamos las probabilidades del ejemplo?

Veamos por ejemplo $P[X=50]$ y $P[X\leq 45].$ Iniciemos con $P[X=50]$.

```{r Poiss distribucion}
lbd=41.8
dpois(50,lambda=lbd) #P[X=50]
...
Vamos a calcular la probabilidad de que en un día calquiera hayan entre 38 y 45 nacimientos.

$$
P[38\leq X \leq 45]=P[X\leq 45]-P[X\leq 37]=0.4646523
$$

```{r calculo probabilidad poisson}
ppois(45,lambda = lbd)-ppois(37,lambda =lbd)
```
