...
ggplot() +
  geom_sf(data=mundocol) +
  geom_sf(data=mapdeptos_Ing,aes(fill=Media_Ing),col="darkgray",linetype="solid") +
  coord_sf(xlim=c(box$xmin,box$xmax),ylim=c(box$ymin,box$ymax),expand=FALSE) +
  geom_sf_text(data=mapdeptos_Ing,aes(label=ifelse(Media_Ing < 1300000,DPTO_CNMBR,"")),col="black",
               fontface="bold",size=4,fun.geometry=function(x) sf::st_centroid(x)) +
...
###Boyacá Estacionaes de Monitoreo Precipitación ----
